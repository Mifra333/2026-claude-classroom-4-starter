...
- **Record card** — a panel whose body is an A2UI surface rather than our own
  markup (`components/project-wizard.tsx`), with the input that drives it
  underneath and a status line under that. The panel, its heading and the
  empty state are ours and follow the rules above; the fields inside are the
  basic catalog's components, drawn by the A2UI renderer with its own
  stylesheet, so there is nothing there to restyle with a Tailwind class. If
  that body ever needs to look like the rest of the app, the lever is the
  `theme` prop on `A2UIProvider` — not a class on the surface.
